...
function timeSpanToString(startDate, endDate) {
    let delta = new Date(endDate.getTime() - startDate.getTime());
    let hour = delta.getUTCHours();
    let minute = delta.getUTCMinutes();
    let second = delta.getUTCSeconds();
    let millisecond = delta.getUTCMilliseconds();
    let temp = "" + ((hour < 10) ? "0" : "") + hour;
    temp += ((minute < 10) ? ":0" : ":") + minute;
    temp += ((second < 10) ? ":0" : ":") + second;
    temp += ((millisecond < 100) ? ".00" : (millisecond < 10) ? ".0" : ".") + millisecond;
    return temp;    
}
...
function angleBetweenClockHands(date) {
    let hour_angle = 0.5 * ((date.getUTCHours() % 12) * 60 + date.getUTCMinutes());
    let minutes_angle = 6 * date.getUTCMinutes();
    let delta_angle = Math.abs(hour_angle - minutes_angle);
    return (delta_angle > 180 ? 360 - delta_angle : delta_angle) * Math.PI / 180;
}
